...
"""
- 전체 칼럼 중에서, numeric과 non-numeric 을 구분한다. 
"""
...
def preprocessingX(input_df):
    r = pd.DataFrame({
        ...
        'TotalSF': input_df['TotalBsmtSF'].fillna(input_df['TotalBsmtSF'].mean()) + input_df['1stFlrSF'].fillna(
            input_df['1stFlrSF'].mean()) + input_df['2ndFlrSF'].fillna(input_df['2ndFlrSF'].mean()),
